...
def test_base_builder_no_python(tmpdir):
...
def test_base_builder_create(clear):
    builder = BaseBuilder("/a/", "", None, None, clear=clear)
    builder.clear_virtual_environment = pretend.call_recorder(lambda: None)
    builder.create_virtual_environment = pretend.call_recorder(lambda: None)
    builder.install_scripts = pretend.call_recorder(lambda: None)
    builder.install_tools = pretend.call_recorder(lambda: None)
    builder.create()

    if clear:
        assert builder.clear_virtual_environment.calls == [pretend.call()]
    else:
        assert builder.clear_virtual_environment.calls == []

    assert builder.create_virtual_environment.calls == [pretend.call()]
    assert builder.install_scripts.calls == [pretend.call()]
    assert builder.install_tools.calls == [pretend.call()]


def test_base_builder_clear_environment_doesnt_exist(tmpdir):
    envdir = str(tmpdir.join("env"))
    builder = BaseBuilder(envdir, None, None, clear=True)
    builder.clear_virtual_environment()
...
def test_base_builder_install_scripts(tmpdir, flavor):
    ...
    builder = BaseBuilder(envdir, None, flavor)
    builder.install_scripts()
